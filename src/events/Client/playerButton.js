const { EmbedBuilder, Client, ButtonInteraction, PermissionsBitField } = require("discord.js");
    ...
    run: async (client, interaction, data) => {
        ...
        if (data2) {
            if (data2.Mode) {
                ...
                if (!pass && !interaction.member.permissions.has(PermissionsBitField.Flags.ManageGuild)) return await buttonReply(interaction, `You don't have dj role to use this command`, color)
            };
        };
        ...
        let nowplaying = new EmbedBuilder().setColor(color).setDescription(`[${player.queue.current.title}](${player.queue.current.uri}) • \`[ ${player.queue.current.isStream ? '[**◉ LIVE**]' : convertTime(player.queue.current.length)} ]\``).setImage(icon).setFooter({ text: `Requested by ${player.queue.current.requester.tag}`, iconURL: player.queue.current.requester.displayAvatarURL({ dynamic: true }) });
